...
MPI_Group_difference (MPI_Group group1, MPI_Group group2, MPI_Group *newgroup)
{
  ...
  if (!address) {
    if ((address = dlsym(MPI_libhandle,"MPI_Group_difference")) == NULL) {
      printf("%s %s %s",SYM_MISSING_PREFIX,"MPI_Group_difference",SYM_MISSING_TRAILER);
      return -1;
    }
  }
  *newgroup = new_index(active_groups);
  if (active_groups->use_ptrs) { api_use_ptrs *local_a0=active_groups->api_declared;
    ...
    int (*VendorMPI_Group_difference)(void *,void *,void **) = address;
    mpi_return = (*VendorMPI_Group_difference)(local_a0[group1].mpi_const,local_a1[group2].mpi_const,&local_a2[*newgroup].mpi_const);
    ...
    int (*VendorMPI_Group_difference)(int,int,int *) = address;
    mpi_return = (*VendorMPI_Group_difference)(local_a0[group1].mpi_const,local_a1[group2].mpi_const,&local_a2[*newgroup].mpi_const);
    if (local_a2[*newgroup].mpi_const == local_a2[MPI_GROUP_NULL].mpi_const) {
      free_index(active_requests,*newgroup);
      *newgroup = MPI_GROUP_NULL;
