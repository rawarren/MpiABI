...
MPI_Reduce_scatter_block (void *sendbuf, void *recvbuf, int recvcount, MPI_Datatype datatype, MPI_Op op, MPI_Comm comm)
{
  ...
  if (active_datatypes->use_ptrs) { api_use_ptrs *local_a0 = active_datatypes->api_declared;
    ...
    int (*VendorMPI_Reduce_scatter)(void *sendbuf,void *recvbuf,int recvcount,void *,void *,void *) = address;
    mpi_return = (*VendorMPI_Reduce_scatter)(INPLACE(sendbuf),INPLACE(recvbuf),recvcount,local_a0[datatype].mpi_const,local_a1[op].mpi_const,local_a2[comm].mpi_const);
  } else { api_use_ints *local_a0 = active_datatypes->api_declared;
    api_use_ints *local_a1=active_ops->api_declared;
    api_use_ints *local_a2= active_comms->api_declared;
    int (*VendorMPI_Reduce_scatter)(void *sendbuf,void *recvbuf,int recvcount,int,int,int) = address;
    mpi_return = (*VendorMPI_Reduce_scatter)(INPLACE(sendbuf),INPLACE(recvbuf),recvcount,local_a0[datatype].mpi_const,local_a1[op].mpi_const,local_a2[comm].mpi_const);
  }
  return mpi_return;
}
