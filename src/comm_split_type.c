...
MPI_Comm_split_type (MPI_Comm comm, int split_type, int color, int key, MPI_Comm *newcomm)
{
  ...
  *newcomm = new_index(active_comms);
  ...
  if (active_comms->use_ptrs) { api_use_ptrs *local_a0= active_comms->api_declared;
    ...
    mpi_return = (*VendorMPI_Comm_split_type)(local_a0[comm].mpi_const,vendor_split_type,UNCOLOR(color),key,&local_a1[*newcomm].mpi_const);
    ...
    int (*VendorMPI_Comm_split_type)(int,int split_type,int color,int key,int *) = address;
    mpi_return = (*VendorMPI_Comm_split_type)(local_a0[comm].mpi_const,vendor_split_type,UNCOLOR(color),key,&local_a1[*newcomm].mpi_const);
    if (local_a1[*newcomm].mpi_const == local_a1[MPI_COMM_NULL].mpi_const) {
      free_index(active_comms,*newcomm);
      *newcomm = MPI_COMM_NULL;
