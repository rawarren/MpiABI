...
MPI_Comm_split_type (MPI_Comm comm, int split_type, int key, MPI_Info info, MPI_Comm *newcomm)
{
  ...
  *newcomm = new_index(active_comms);
  ...
  if (active_comms->use_ptrs) { api_use_ptrs *local_a0= active_comms->api_declared;
	...
	int (*VendorMPI_Comm_split_type)(void *,int split_type,int key,void *,void **) = address;
	mpi_return = (*VendorMPI_Comm_split_type)(local_a0[comm].mpi_const,vendor_split_type,key,local_a2[info].mpi_const,&local_a1[*newcomm].mpi_const);
    } else { api_use_ints *local_a2=active_infos->api_declared;
	int (*VendorMPI_Comm_split_type)(void *,int split_type,int key,int,void **) = address;
	mpi_return = (*VendorMPI_Comm_split_type)(local_a0[comm].mpi_const,vendor_split_type,key,local_a2[info].mpi_const,&local_a1[*newcomm].mpi_const);
    }
    ...
    if (active_infos->use_ptrs) { api_use_ptrs *local_a2=active_infos->api_declared;
	int (*VendorMPI_Comm_split_type)(int,int split_type,int key,void *,int *) = address;
	mpi_return = (*VendorMPI_Comm_split_type)(local_a0[comm].mpi_const,vendor_split_type,key,local_a2[info].mpi_const,&local_a1[*newcomm].mpi_const);
    } else { api_use_ints *local_a2=active_infos->api_declared;
	int (*VendorMPI_Comm_split_type)(int,int split_type,int key,int, int *) = address;
	mpi_return = (*VendorMPI_Comm_split_type)(local_a0[comm].mpi_const,vendor_split_type,key,local_a2[info].mpi_const,&local_a1[*newcomm].mpi_const);
    }
    if (local_a1[*newcomm].mpi_const == local_a1[MPI_COMM_NULL].mpi_const) {
      free_index(active_comms,*newcomm);
