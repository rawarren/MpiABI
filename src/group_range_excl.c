...
MPI_Group_range_excl (MPI_Group group, int n, int ranges[][3], MPI_Group *newgroup)
{
  ...
  if (!address) {
    if ((address = dlsym(MPI_libhandle,"MPI_Group_range_excl")) == NULL) {
      printf("%s %s %s",SYM_MISSING_PREFIX,"MPI_Group_range_excl",SYM_MISSING_TRAILER);
      return -1;
    }
  }
  *newgroup = new_index(active_groups);
  ...
  if (active_groups->use_ptrs) { api_use_ptrs *local_a0=active_groups->api_declared;
    ...
    int (*VendorMPI_Group_range_excl)(void *,int n, int ranges[][3], void **) = address;
    mpi_return = (*VendorMPI_Group_range_excl)(local_a0[group].mpi_const,n,ranges,&local_a1[*newgroup].mpi_const);
    ...
    int (*VendorMPI_Group_range_excl)(int,int n, int ranges[][3], int *) = address;
    mpi_return = (*VendorMPI_Group_range_excl)(local_a0[group].mpi_const,n,ranges,&local_a1[*newgroup].mpi_const);
    if (local_a0[*newgroup].mpi_const == local_a0[MPI_GROUP_NULL].mpi_const) {
      free_index(active_groups,*newgroup);
      *newgroup = MPI_GROUP_NULL;
