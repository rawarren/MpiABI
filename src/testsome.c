...
MPI_Testsome (int count, MPI_Request array_of_requests[], int *outcount, int array_of_indices[], MPI_Status *array_of_statuses)
{
    ...
    if (active_requests->use_ptrs) { api_use_ptrs *local_a0=active_requests->api_declared;
        ...
        if (array_of_statuses == MPI_STATUS_IGNORE) {
			...
			if (active_count)
				completed = *outcount;
			else if (*outcount == local_a2[ISC_UNDEFINED_].mpi_const)
				*outcount = ISC_UNDEFINED;
            ...
            for(i=0; i< completed; i++) {
                if (mpi_return == 0)
					native_status_to_isc_no_error(1,&natstat[(i*elems)],(int *)&array_of_statuses[i]);
                else native_status_to_isc(1,&natstat[(i*elems)],(int *)&array_of_statuses[i]);
