...
MPI_Waitany (int count, MPI_Request *array_of_requests, int *index, MPI_Status *status)
{
    ...
    if (active_requests->use_ptrs) { api_use_ptrs *local_a0=active_requests->api_declared;
     int (*VendorMPI_Waitany)(int count,void **,int *index, MPI_Status *status) = address;
     mpi_return = (*VendorMPI_Waitany)(count,&local_a0[*array_of_requests].mpi_const,index,status);
    } else { api_use_ints *local_a0=active_requests->api_declared;
     int (*VendorMPI_Waitany)(int count,int *,int *index, MPI_Status *status) = address;
     mpi_return = (*VendorMPI_Waitany)(count,&local_a0[*array_of_requests].mpi_const,index,status);
   }
    return mpi_return;
}
