...
MPI_File_read_all (MPI_File fh, void *buf, int count, MPI_Datatype datatype, MPI_Status *status)
{
  ...
  if (!address) {
    if ((address = dlsym(MPIO_libhandle,"MPI_File_read_all")) == NULL) {
      printf("%s %s %s",SYM_MISSING_PREFIX,"MPI_File_read_all",SYM_MISSING_TRAILER);
      return -1;
    }
  }
  if (active_files->use_ptrs) { api_use_ptrs *local_a0=active_files->api_declared;
    ...
    int (*VendorMPI_File_read_all)(void *,void *buf,int count,void *, MPI_Status *status) = address;
    mpi_return = (*VendorMPI_File_read_all)(local_a0[fh].mpi_const,buf,count,local_a1[datatype].mpi_const,SIGNORE(status));
    ...
    int (*VendorMPI_File_read_all)(int,void *buf,int count,int, MPI_Status *status) = address;
    mpi_return = (*VendorMPI_File_read_all)(local_a0[fh].mpi_const,buf,count,local_a1[datatype].mpi_const,SIGNORE(status));
    if (status != MPI_STATUS_IGNORE) native_status_to_isc(1,natstat,(int *)status);
