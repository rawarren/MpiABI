...
MPI_Comm_accept (char *port_name, MPI_Info info, int root, MPI_Comm comm, MPI_Comm *newcomm)
{
  ...
  if (!address) {
    if ((address = dlsym(MPI_libhandle,"MPI_Comm_accept")) == NULL) {
      printf("%s %s %s",SYM_MISSING_PREFIX,"MPI_Comm_accept",SYM_MISSING_TRAILER);
      return -1;
    }
  }
  *newcomm = new_index(active_comms);
  if (active_infos->use_ptrs) { api_use_ptrs *local_a0=active_infos->api_declared;
    ...
    int (*VendorMPI_Comm_accept)(char *port_name,void *,int root,void *,void **) = address;
    mpi_return = (*VendorMPI_Comm_accept)(port_name,local_a0[info].mpi_const,root,local_a1[comm].mpi_const,&local_a2[*newcomm].mpi_const);
    ...
    int (*VendorMPI_Comm_accept)(char *port_name,int,int root,int,int *) = address;
    mpi_return = (*VendorMPI_Comm_accept)(port_name,local_a0[info].mpi_const,root,local_a1[comm].mpi_const,&local_a2[*newcomm].mpi_const);
    if (local_a2[*newcomm].mpi_const == local_a2[MPI_COMM_NULL].mpi_const) {
      free_index(active_comms,*newcomm);
      *newcomm = MPI_COMM_NULL;
