...
MPI_Scatterv (void *sendbuf, int sendcounts[], int displs[], MPI_Datatype sendtype, void *recvbuf, int recvcount, MPI_Datatype recvtype, int root, MPI_Comm comm)
{
  ...
  if (active_datatypes->use_ptrs) { api_use_ptrs *local_a0 = active_datatypes->api_declared;
    ...
    mpi_return = (*VendorMPI_Scatterv)(INPLACE(sendbuf),sendcounts,displs,local_a0[sendtype].mpi_const,INPLACE(recvbuf),recvcount,local_a1[recvtype].mpi_const,vendor_root,local_a2[comm].mpi_const);
