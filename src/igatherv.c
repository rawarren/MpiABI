  ...
  *request = new_index(active_requests);
  ...
  if (active_datatypes->use_ptrs) { api_use_ptrs *local_a0 = active_datatypes->api_declared;
    ...
    if (active_requests->use_ptrs) { api_use_ptrs *local_a3=active_requests->api_declared;
      ...
      mpi_return = (*VendorMPI_Igatherv)(INPLACE(sendbuf),sendcount,local_a0[sendtype].mpi_const,INPLACE(recvbuf),recvcounts,displs,local_a1[recvtype].mpi_const,root,local_a2[comm].mpi_const,&local_a3[*request].mpi_const);
    }
